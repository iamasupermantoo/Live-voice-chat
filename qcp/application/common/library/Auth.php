...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id','nickname','headimgurl','ry_uid','ry_token','phone'];

    public function __construct($options = [])
    {
        if ($config = Config::get('users')) {
     ...
     * @return boolean
     */
    public function register($password,$mobile = '',$nickname,$sex,$headimgurl,$birthday,$system,$channel)
    {
        // 检测手机号是否存在
        if ($mobile && Users::get(['phone' => $mobile])) {
            $this->setError('Mobile already exist');
            return false;
        }

        if(!isPassword($password)){
            $this->setError('密码为长度6-20位的数字或字母混合组成!');return false;
        }

        $salt = substr(md5(time()),0,3);$salt = '123';
        ...
        $data = [
            ...
            'sex'           => $sex,
            'birthday'      => $birthday,
            'constellation' => getBrithdayMsg($birthday,2),
            'salt'          => $salt,
            'pass'          => $password,
            'nickname'      => $nickname,
        ...
        Db::startTrans();
        try {
            $user = Users::create($data, true);
            $this->_user = Users::get($user->id);
            // if($user->id){
            //     $img = $this->setFilePath($user->headimgurl);
            //     $res = $this->ryHand(1,$user->id,$user->nickname,$img);
            //     if($res['code'] == 200){
            //         DB::name('users')->where('id',$user->id)->update(['ry_uid'=>$user->id,'ry_token'=>$res['token']]);
            //     }
            // }
            //设置Token
    ...
    {
        //$field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? 'phone' : 'nickname');
        $field = 'phone';
        $user = Users::get([$field => $account]);
        if (!$user) {
            $this->setError('该手机号没有注册，请先去注册');
            return false;
        }

        if ($user->pass != $this->getEncryptPassword($password, $user->salt)) {
            $this->setError('账号或密码有误');
            return false;
        }

        if ($user->status == '2') {
            if($user->locktime > time()){
                $da =  date('Y-m-d H:i:s',$user->locktime);
                $this->setError('账号已被锁定,请于'.$da.'之后登陆');
                return false;
            }
        }
        //accid存在更新ry_uid
        if($user->ry_uid){
            ...
            if($res['code'] == 200){
                DB::name('users')->where('id',$user->id)->update(['ry_token'=>$res['token']]);
            }
        }
    ...
    protected function ryHand($type,$ry_uid,$nickname='',$headimg=''){
        ...
        $AppKey = Configs::getConfigByName('ry_app_key');
        $AppSecret = Configs::getConfigByName('ry_app_secret');
...
/*************************************************/
    ...
    public function getUserinfo()
    {
        ...
        $userinfo['headimgurl'] = $this->setFilePath($userinfo['headimgurl']);
        return $userinfo;
    }
