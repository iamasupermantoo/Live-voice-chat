...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        /*本地上传,自定义文件夹  chufang 2019-07-29
         */
        ...
        $uploadDir = $specialpath ? $myfath : substr($savekey, 0, strripos($savekey, '/') + 1);

        $fileName = substr($savekey, strripos($savekey, '/') + 1);

        $uploadDir_data = $uploadDir;
        $uploadDir = $upload['rootpath'].$uploadDir;
        //
        $splInfo = $file->validate(['size' => $size])->move(ROOT_PATH . '/public' . $uploadDir, $fileName);
        //echo $uploadDir_data;die;
        if ($splInfo) {
            $params = array(
            ...
            $this->success(__('Upload successful'), null, [
                'url' => $uploadDir . $splInfo->getSaveName()
                //'url' => $uploadDir_data . $splInfo->getSaveName()
            ]);
        } else {
            // 上传失败获取错误信息
